...
export default function fr0Page() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      const res = await fetch(`/api/fr0/stats?stationId=${stationId}`);
      if (res.ok) {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "url('/images/fr0-bg.png')",
          ...
          <label className="block">
            ...
            >
              ...
              {Array.from({ length: 30 }, (_, i) => {
                const code = i + 1 < 10 ? `FS0${i + 1}` : `FS${i + 1}`;
                return (
